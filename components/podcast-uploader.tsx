...
export const PodcastUploader = () => {
  ...
  const handleFileSelect = async (file: File) => {
    ...
    setUploadStatus(PODCAST_UPLOAD_STATUS.IDLE);
    setUploadProgress(0);
    setError(null);
  ...
  const handleUpload = async () => {
    ...
    try {
      setUploadStatus(PODCAST_UPLOAD_STATUS.UPLOADING);
      setUploadProgress(0);
      ...
      setUploadStatus(PODCAST_UPLOAD_STATUS.PROCESSING);
      ...
      setUploadStatus(PODCAST_UPLOAD_STATUS.COMPLETED);

      // Step 4: Navigate to project detail page
      router.push(`/dashboard/projects/${projectId}`);
    } catch (err) {
      console.error('Upload error:', err);
      setUploadStatus(PODCAST_UPLOAD_STATUS.ERROR);
  ...
  return (
    <div className='space-y-6'>
      ...
      {!selectedFile && uploadStatus === PODCAST_UPLOAD_STATUS.IDLE && (
        <UploadDropzone
          onFileSelect={handleFileSelect}
          disabled={uploadStatus !== PODCAST_UPLOAD_STATUS.IDLE}
        />
      )}
      ...
      {selectedFile && (
        <>
          ...
          {(uploadStatus === PODCAST_UPLOAD_STATUS.IDLE ||
            uploadStatus === PODCAST_UPLOAD_STATUS.ERROR) && (
