...
import { isMockTranscription } from '@/lib/_tests_/mock-flags';
...
export type AssemblyAIWebhookData = {
  transcriptId: string;
  status: string;
  error?: string;
};

export const parseAssemblyAIWebhook = (
  body: Record<string, unknown>
): AssemblyAIWebhookData | null => {
  const transcriptId =
  ...
  if (!transcriptId || !status) return null;

  return { transcriptId, status, error };
};

const buildAssemblyAIWebhookUrl = (projectId: Id<'projects'>): string => {
...
const toSpeakers = (utterances: AssemblyAIUtterance[] = []) =>
...
const applyMockTranscript = async (
  ...
  await convex.mutation(api.projects.saveTranscript, {
    ...
    transcript: {
      ...
      speakers: toSpeakers(transcript.utterances),
...
export const transcribePodcast = async (
  step: StepTools,
  audioUrl: string,
  projectId: Id<'projects'>,
  ...
  if (isMockTranscription()) {
    return applyMockTranscript(projectId);
  }
  ...
  try {
    ...
    if (!step.waitForEvent) {
      throw new Error('waitForEvent is required for AssemblyAI transcription');
    }
    ...
    const speakers = toSpeakers(assemblyUtterances);
