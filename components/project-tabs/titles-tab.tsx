    ...
    <div className='space-y-6'>
      {TITLE_CATEGORIES.map((category) => (
        <div key={category.key} className='glass-card rounded-2xl p-6 md:p-8'>
          <h3 className='text-xl md:text-2xl font-bold mb-4 md:mb-6 gradient-sunrise-text'>
            {category.title}
          </h3>
          {category.type === 'list' ? (
            ...
            <div className='flex flex-wrap gap-2 md:gap-3'>
              {titles[category.key].map((keyword) => (
                <Badge
                  key={keyword}
                  className='px-3 md:px-4 py-1.5 md:py-2 text-xs md:text-sm gradient-sunrise text-white shadow-md wrap-break-word'
                >
                  {keyword}
                </Badge>
              ))}
